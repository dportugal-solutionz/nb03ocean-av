...
PIMCO [Endpoint Configuration](https://github.com/brianlopezpimco/nb03ocean-av#endpoint-configuration-files) (backups) have all the required configuration settings as well as the "Join Zoom CRC" application (JavaScript Macros & UI Extension XML) already configured. To replace an endpoint or configure a new one; restore the unit to factory defaults settings, upgrade the unit to current firmware (RoomOS11), then upload the appropriate configuration file following the [Configuration Files Installation](https://github.com/brianlopezpimco/nb03ocean-av#configuration-file-installation) section below.
